...
@section('page-title', 'Kelola Anggota Household')
...
<div class="col-12 col-lg-8">
    ...
    <div class="card border-0 shadow-sm" style="border-radius:.75rem;">
        <div class="card-header bg-white border-bottom py-3 px-4" style="border-radius:.75rem .75rem 0 0;">
            <h6 class="fw-semibold mb-0">Anggota Aktif ({{ $members->count() }})</h6>
        </div>
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        @if(auth()->user()->role === 'owner' && $member->id !== auth()->id() && $member->id !== $household->owner_id)
                            <form method="POST" action="{{ route('household.members.remove', $member) }}"
                                  onsubmit="return confirm('Hapus {{ $member->name }} dari household?')"
                                  class="d-inline">
                                @csrf @method('DELETE')
    ...
    @if($invitations->isNotEmpty())
        <div class="card border-0 shadow-sm" style="border-radius:.75rem;">
            <div class="card-header bg-white border-bottom py-3 px-4" style="border-radius:.75rem .75rem 0 0;">
                <h6 class="fw-semibold mb-0">Undangan Menunggu ({{ $invitations->count() }})</h6>
            </div>
            <div class="card-body p-0">
    ...
    @if(in_array(auth()->user()->role, ['owner', 'admin']))
        <div class="card border-0 shadow-sm" style="border-radius:.75rem;">
            <div class="card-body p-4">
                <h6 class="fw-semibold mb-3">Undang Anggota Baru</h6>
