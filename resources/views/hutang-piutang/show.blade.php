...
<div class="col-12 col-lg-8">
    ...
    <div class="card border-0 shadow-sm mb-3" style="border-radius:.75rem;">
        <div class="card-body p-3 p-md-4">
                ...
                <div>
                    <div class="d-flex align-items-center gap-2 mb-1">
                        ...
                        @if($hutangPiutang->tipe_pembayaran === 'cicilan')
                            <span class="badge rounded-pill bg-info text-dark" style="font-size:.65rem;">
                                {{ __('hutang.payment_installment') }}
                            </span>
                        @endif
                    </div>
                    <div class="fw-bold">{{ $hutangPiutang->nama_pihak }}</div>
                    @if($hutangPiutang->keterangan)
                        <div class="text-muted small">{{ $hutangPiutang->keterangan }}</div>
                    @endif
                </div>
                <div class="d-flex gap-3 ms-3 flex-shrink-0">
            ...
            <div class="row g-2 mb-3">
                <div class="col-4">
                    <div class="p-2 rounded" style="background:var(--bs-tertiary-bg, #f8f9fa);">
                        <div class="text-muted mb-1" style="font-size:.68rem;">{{ __('laporan.total') }}</div>
                        <div class="fw-bold" style="font-size:.88rem;">Rp {{ number_format($hutangPiutang->jumlah_total, 0, ',', '.') }}</div>
                    </div>
                </div>
                <div class="col-4">
                    <div class="p-2 rounded" style="background:var(--bs-tertiary-bg, #f8f9fa);">
                        <div class="text-muted mb-1" style="font-size:.68rem;">{{ __('hutang.paid') }}</div>
                        <div class="fw-bold text-success" style="font-size:.88rem;">Rp {{ number_format($hutangPiutang->jumlah_terbayar, 0, ',', '.') }}</div>
                    </div>
                </div>
                <div class="col-4">
                    <div class="p-2 rounded" style="background:var(--bs-tertiary-bg, #f8f9fa);">
                        <div class="text-muted mb-1" style="font-size:.68rem;">{{ __('hutang.outstanding') }}</div>
                        <div class="fw-bold {{ $hutangPiutang->jenis === 'hutang' ? 'text-danger' : 'text-primary' }}" style="font-size:.88rem;">
            ...
            <div class="d-flex justify-content-between" style="font-size:.7rem; color:var(--bs-secondary-color, #6c757d);">
            ...
            {{-- Cicilan info (inline, no separate card) --}}
            @if($hutangPiutang->tipe_pembayaran === 'cicilan' && $hutangPiutang->status !== 'lunas')
                <div class="mt-2 pt-2 border-top d-flex align-items-center gap-2 small text-info">
    ...
    @if($hutangPiutang->status !== 'lunas')
        <div class="card border-0 shadow-sm mb-3" style="border-radius:.75rem;">
            <div class="card-body p-3 p-md-4">
                <h6 class="fw-semibold mb-3" style="font-size:.88rem;">
                ...
                <form method="POST" action="{{ route('hutang-piutang.bayar', $hutangPiutang) }}">
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-sm-6">
                            <label class="form-label small fw-medium mb-1">{{ __('hutang.source') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="col-sm-6">
                            <label class="form-label small fw-medium mb-1">{{ __('hutang.amount') }} <span class="text-danger">*</span></label>
                            <div class="input-group input-group-sm">
                                <span class="input-group-text">Rp</span>
                                <input type="number" name="jumlah" min="1" step="any" max="{{ $hutangPiutang->sisa }}" required
                                       value="{{ $hutangPiutang->tipe_pembayaran === 'cicilan' ? $hutangPiutang->jumlah_cicilan : $hutangPiutang->sisa }}"
    ...
    <div class="card border-0 shadow-sm" style="border-radius:.75rem;">
        <div class="card-header bg-transparent border-bottom py-2 px-3" style="border-radius:.75rem .75rem 0 0;">
            <h6 class="fw-semibold mb-0" style="font-size:.85rem;">{{ __('laporan.transactions') }}</h6>
        </div>
        <div class="card-body p-0">
            @forelse($riwayat ?? [] as $r)
                <div class="d-flex align-items-center gap-2 px-3 py-2 border-bottom">
                    <div class="flex-grow-1 min-width-0">
                        <div class="small fw-medium text-truncate">{{ $r->keterangan ?: __('hutang.payment') }}</div>
                    ...
                    <div class="d-flex gap-1 flex-shrink-0">
                        ...
                        <form method="POST" action="{{ route('pembayaran.destroy', $r) }}"
                            ...
                            @csrf @method('DELETE')
                            <button type="submit" class="btn btn-outline-danger btn-sm py-0 px-2" style="font-size:.7rem;">
                                {{ __('messages.delete') }}
                            </button>
                        </form>
                    </div>
